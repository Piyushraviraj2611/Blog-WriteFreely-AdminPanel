...
const PostCard = ({ post, onDeleteClick }) => {
    ...
    if (!post) return null;
    const { title, thumbnail, tags, meta, createdAt, slug, content } = post;

    return (
        <div>
            <div className="post-card bg-white shadow-sm rounded flex flex-col">
                <div className="post-card-image">
                 {   <img className='aspect-video' src={thumbnail ? thumbnail.url : "https://propertywiselaunceston.com.au/wp-content/themes/property-wise/images/no-image.png"} alt={title} />
 } </div>
                <div className="post-card-content p-2 flex flex-col flex-1 justify-between">
                    ...
                    <div className="post-card-meta">
                        <p className="text-gray-500">{meta && meta.length > 80 ? meta.substring(0, 75) + "..." : meta}</p>
                    </div>
                    ...
                    <div className="flex space-x-3">
                        <button className='w-8 h-8 rounded-full bg-green-500 flex justify-center items-center text-white hover:bg-green-600'
                        ...
                        <button className='w-8 h-8 rounded-full bg-red-400 flex justify-center items-center text-white hover:bg-red-600'
                            onClick={onDeleteClick}>
                            <BsTrash />
                        </button>
                    </div>

                    <MobileView title={title} content={content} thumbnail={ thumbnail && thumbnail.url} visible={displayMobileView} onClose={() => setDisplayMobileView(false)} />
