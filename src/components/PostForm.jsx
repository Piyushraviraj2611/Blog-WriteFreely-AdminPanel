...
const PostForm = ({
	...
	const handleChange = (e) => {
		...
		if (name === 'featured') {
			localStorage.setItem(
				'post',
				JSON.stringify({ ...post, featured: e.target.checked })
			);
		...
		if (name === 'meta' && value.length > 150) {
			updateNotification(
				'warning',
				"Meta description can't be more than 150 characters"
			);
	...
	const handleSubmit = (e) => {
		...
		if (!post.title.trim())
			return updateNotification('error', 'Title is required');
		...
		if (!post.tags.length)
			return updateNotification('error', 'Tags are required');
		if (!post.meta.trim())
			...
			<form className="p-2 flex" onSubmit={handleSubmit}>
				<div className="w-9/12 h-screen space-y-3 flex flex-col">
					...
					<div className="flex items-center space-x-2 mt-5">
						...
						<label
							htmlFor="featured"
							className="select-none text-gray-700 cursor-pointer group"
						>
					...
					<textarea
						...
						cols="30"
						rows="10"
						className="flex-1 w-full p-2 h-96 border-2 font-mono tracking-wide text-lg border-gray-300 rounded focus:outline-none focus:border-blue-500 resize-none"
					></textarea>
					{/* Tags Input */}
					<div>
					...
					<div>
						...
						<textarea
							...
							cols="30"
							rows="3"
							className="w-full p-2 h-28 border-2 border-gray-300 rounded focus:outline-none focus:border-blue-500 resize-none"
						></textarea>
					</div>
				</div>
